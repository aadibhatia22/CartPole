...
import math
from models.state_action_pair import StateActionPair
from models.agent_actions import AgentAction
from models.zl_al import ZlAl
import random
class NeuralNetwork():
    ...
    def __init__(self, learning_rate:float, discount_factor:float, architecture: np.ndarray,
                 random_number1:float =-1.0, random_number2:float=1.0, epsilon_start:float = 1.0, epsilon_end:float = 0.01, epsilon_decay_rate: float = 0.999 ):
    ...
    #method will be called at the end of the backpropagation_training method and this will creat the state action pairs
    #then after the episode finishes the addReward will give reward to each action adn then after that
    #we will run back propogaion method that will run back prop for each action from the episode\
    def record_action(self, input:np.ndarray, actionTaken: AgentAction, probability_of_left, reward:float = None, ZlAl_input: ZlAl = None):

        # LEARN MORE ABOUT d_log_pt
        if(actionTaken == AgentAction.LEFT):
            self.state_action_pairs.append(StateActionPair(input, actionTaken, AgentAction.LEFT - probability_of_left, ZlAl_input))
        if(actionTaken == AgentAction.RIGHT):
            self.state_action_pairs.append(StateActionPair(input, actionTaken, AgentAction.Right - probability_of_left, ZlAl_input))




    #ADD EPISLION FOR RANDOM ACTION
    #Will return the action for the state, and also store
    def backpropagation_training(self,input:np.array):
        zl_al = ZlAl()
        ...
        for i in range(len(self.architecture)):
            if i == 0:
                ...
                # @ does matrix multiplication
                zl = self.weight[i]@input + self.bias[i]
                if(i!=len(self.architecture)-1):
                    #RELU
                    al = np.where(zl <0, 0, zl)
                    zl_al.add(zl,al)
                    input = al;
                elif(i==len(self.architecture)-1):
                    #SIGMOID
                    al = np.where(zl <0, 0, zl)
                    probability_of_left = self.sigmoid(al)
                    zl_al.add(zl,probability_of_left)
                    random =random.random()
                    if(random<=probability_of_left):
                        ##check the zl_al instances
                        self.record_action(input, AgentAction.LEFT, probability_of_left, None, zl_al)
                        return AgentAction.LEFT
                    else:   
                        self.record_action(input, AgentAction.RIGHT, probability_of_left, None, zl_al)
    ...
    def addReward(self, reward_list: list):
        #Looping over episodes
        #in ZlAl the first index is null
        for i in range (len(reward_list)):
            self.state_action_pairs[i].addReward(reward_list(i))
        return None;
